...
public class ReelServiceDAOImpl implements ReelServiceDAO {
    ...
    public boolean addReel(final Reel reel, final Integer userId) {
        try (final PreparedStatement preparedStatement = connection.prepareStatement(String.join(" ", "INSERT INTO REELS  (user_id, caption," + "duration) VALUES (?, ?, ?)"), Statement.RETURN_GENERATED_KEYS)) {
     ...
     */
    public boolean removeReel(final int postId, final Integer userId) {
        try (final PreparedStatement preparedStatement = connection.prepareStatement(String.join(" ", "DELETE FROM REELS WHERE reels.id = ?"))) {
